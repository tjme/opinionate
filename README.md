...
If you would like to (partially or fully) develop in a containerized environment, see [opinionate-docker](https://github.com/tjme/opinionate-docker).
...
\q
```
- Run PostGraphile (in the background) with: `postgraphile -c postgres://test:testpass@/pagila --cors --enhance-graphiql -n localhost -p 5000 -q /graphql &` (or later, with `pnpm backend`)
...
- Create models directory (e.g. with `mkdir models`), to store files about database metadata
- generate everything: `pnpm gen` (or also build and run a dev server with `pnpm genDev`), OR do it stage by stage with:
  - create a data sources directory: `mkdir models`
  - install dependencies: `pnpm install`
  - generate JSON schema file: `pnpm gen-json`
  - optionally generate GraphQL schema file: `pnpm gen-gql`
