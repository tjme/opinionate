...
+(entity.fields.filter(f => getType(f)!=null && !["`+entity.primaryLabel+`"].includes(f.name)).map(fields => fields.name+(isField(fields) ? "" : "{totalCount}")))+` }\`;
  ...
  const Create = gql\`mutation create(`+entity.fields
    .filter(f => isField(f) && !["nodeId","`+entity.primaryLabel+`"].includes(f.name)).map(field => '$'+field.name+':'+(field.meta.array ? '['+getType(field)+']' : getType(field))+(field.meta.required && !field.meta.readonly ? "!" : ""))+`)
    {create`+entity.name+`(input:{`+to1LowerCase(entity.name)+`:{ `+entity.fields
        .filter(f => isField(f) && !["nodeId","`+entity.primaryLabel+`"].includes(f.name)).map(field => field.name+':\$'+field.name)+` } })
...
`+entity.fields.filter(f => isField(f) && f.meta.linkEntity && !f.meta.linkFields).map(f => f.meta).flat().filter((obj, index, self) => index === self.findIndex((o) => o.linkEntity === obj.linkEntity)).map(m => '\
  ...
  +(entities.filter(l => m.linkEntity==l.name)[0].meta.primaryLabel==entity.primaryLabel ? '(orderBy: _ID_ASC) {nodes{ '+entity.primaryLabel+',' : ' {nodes{ ' )+m.linkFieldsFrom+' }}}\`});\n\
