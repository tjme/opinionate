    ...
    function mergeMeta(item, overlay) {
        ...
        if (item.description) {
            ...
            if (meta && !ignoreComments) {
                item[metaProp] = utils.merge(item[metaProp], relaxedStructure, JSON.parse(utils.convert(meta)));
            }
        ...
        if (overlay) {
            ...
            if (overlayItem && overlayItem[metaProp])
                item[metaProp] = utils.merge(item[metaProp], relaxedStructure, overlayItem[metaProp]);
        }
    }
    ;
